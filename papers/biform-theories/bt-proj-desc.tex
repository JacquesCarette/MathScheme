...
what happens for negative integers).  \unsure{I don't understand the
  meaning or purpose of these last two sentences. It is too esoteric
  for a project description.}{On top of that}, Maple also offers
\texttt{ifactor} (without the 's') that returns an abomination that
displays nicely: a product of a unit and the function with name
\emph{the empty string} applied to a prime, raised to its
multiplicity. This horrendous hack has the advantage of displaying the
result in a way similar to what appears in elementary textbooks.
...
\unsure{What feature is being referred to.}{Maple calls this feature}
an \emph{inert form}, while in Mathematica it is a \emph{hold form},
but the idea is the same: representing a computation without actually
performing it. Such features are \unsure{Are these ``very old''
  features included in the ``new developments''?}{very old}: even in
...
other cases we are interested in it for it syntactic structure.
...
\change{Jacques, I understand why you want to write CASes, but it is
  not standard and, for that reason, confusing.}{CASs} obtain
...
\medskip
  ...
  \item[]\textbf{Implement} Produce an implementation \mname{Impl} of
...
\medskip
  ...
  \item[]\textbf{Theory Graphs} Enable theory graphs of biform theories
    to be defined in \mname{Impl}.  Use combinators to ease the
