...
The first step is to define a theory $T_0 = (L_0,\Gamma_0)$ for the
field $\QQ(x)$ of rational expressions in $x$ over $\QQ$.  $L_0$
contains a base type $\kappa$ that represents the elements of
$\QQ(x)$, a constant $X_\kappa$ which represents the indeterminant
symbol $x$ of $\QQ(x)$, and constants $0_\kappa$, $1_\kappa$,
$+_{\kappa \tarrow \kappa \tarrow \kappa}$, $*_{\kappa \tarrow \kappa
  \tarrow \kappa}$, $-_{\kappa \tarrow \kappa}$, and
$\phantom{}^{{-1}_{\kappa \tarrow \kappa}}$ representing the usual
field elements and operators.  $\Gamma_0$ contains axioms that say the
type $\kappa$ is a field.

The next step is to extend $T_0$ to a theory $T_1 =
(L_1,\Gamma_1)$ by defining in $T_0$ the following constants:
...
rational expressions.  And (5) says that if the input does not
