...
$f' \in \Langre$ as output such that $f'$ is the normal form of $f$.
...
function equals 0 in the corresponding field of fractions.  However,
...
with $q$ is not a homomorphism!  In particular, it $x/x$ is defined in
...
{\NRF} can sensitive to definedness considerations.  We can, however,
specify these algorithm, as we have shown, in a logic like
{\churchqe}.
...
output is also a member of $\mname{DiffExpr}_{\epsilon \tarrow
  o}$. (4--6) say that, if the input is a member of
$\mname{DiffExpr}_{\epsilon \tarrow o}$ and, for all real numbers $a$,
if the function $f$ represented by $u_e$ is differentiable at $a$,
then $f$ equals the function represented by $\mname{diff}_{\epsilon
  \tarrow \epsilon} \, u_e$ at $a$.  And (7) says that, if the input
...
syntax and semantics that is inherent in these algorithm.  For the
