...
{\qzero}'s proof system in~\cite{Farmer18}). Thus every {\HL} proof
remains valid. $\Box$ 

One way to test the implementation of {\HLQE}'s proof system is to
check whether proofs that work in {\HL} continue to work. This is
true for all the proofs are loaded as part of the
{\HL} system. We can further test the implementation by loading
a variety of large {\HL} proofs.
...
definitions of the type $\epsilon$ and constants and
interpretations for the quasiquotation and evaluation operators.
These additions do not impinge upon the semantics of {\HL},
hence every model of the former can be expanded to a model of
the latter.  $\Box$
...
\end{lstlisting}
that is readily proved.  Instances of this are obtained by
applying \texttt{INST} followed by
