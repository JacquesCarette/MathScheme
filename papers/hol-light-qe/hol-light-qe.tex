...
\end{lstlisting}
that is readily proved in {\HLQE}.  An instance of this theorem is
obtain by applying the rule of inference \texttt{INST} followed by the
...
support for inductive types.  {\HLQE} works in exactly the same way as
{\HL} except that, in the presence of evaluations, the instantiation
of free variables may require proving side conditions that say (1) a
variable is not effective in a term or (2) that a variable represented
by a construction is not free in a term represented by a construction
(see subsections~\ref{subsec:cttqe-proof-system}
...
algorithm for symbolic differentiation of polynomials, writing its
