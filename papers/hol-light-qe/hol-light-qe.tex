...
rules, this gives a certain amount of \emph{logical
  reflection}. Evaluation adds to this some aspects of
{\emph{computational reflection}.
=======
...
>>>>>>> 4e6b69c671c801229a1a0d7fc5fc46000baa3e87
...
Quasiquotation provides a convenient way to construct big quotations
from little quotations.  It can be defined straightforwardly in
...
As an example, consider the {\churchqe} quasiquotation
$\synbrack{\Neg(\textbf{A}_o \And
  \commabrack{\textbf{B}_\epsilon})}$. $\commabrack{\textbf{B}_\epsilon}$
is a \emph{hole} or \emph{antiquotation}. Let assume that
$\textbf{A}_o$ contains no holes.  $\synbrack{\Neg(\textbf{A}_o \And
  \commabrack{\textbf{B}_\epsilon})}$ is then an abbreviation for the
significantly more verbose expression
...
\bi
  ...
  \item[] \textbf{B8 (Properties of Quotation)}
  ...
  \item[] \textbf{B10 (Properties of Evaluation)}
  ...
  \be
    ...
    \item $(\mname{is-expr}_{\epsilon \tarrow o}^{\alpha \tarrow \beta}
      ...
      \, \textbf{B}_\epsilon) \Implies\\[.5ex]
      \hspace*{2ex} \sembrack{\mname{app}_{\epsilon \tarrow \epsilon
          \tarrow \epsilon} \, \textbf{A}_\epsilon \,
...
\bi
  ...
  \item[] \textbf{B9 (Beta-Reduction for Quotations)}
  ...
  \item[] \textbf{B11 (Beta-Reduction for Evaluations)}
  ...
  \item[] \textbf{B12 (``Not Free In'' means ``Not Effective In'')}
  ...
  \item[] \textbf{B13 (Beta-Reduction for Function Abstractions)}
...
to the Variable Problem mentioned above, it is only possible to
syntactically check whether a ``variable is not free in an
expression'' when the expression is eval-free.  Our solution to this
problem is to replace the syntactic notion of ``a variable is free in
...
\[\Neg(\mname{is-free-in}_{\epsilon \tarrow \epsilon \tarrow o}
      \, \synbrack{\textbf{x}_\alpha} \, ((\LambdaApp
      \textbf{x}_\alpha \mdot \textbf{B}_\epsilon) \,
      \textbf{A}_\alpha))\] in B11.2 guarantees that there is not a
      double substitution as mentioned above in the Double
      Substitution Problem. $\mname{is-free-in}_{\epsilon \tarrow
...
The system is based on a slightly out-of-date version of {\HL} and
consequently does not work with recent versions of OCaml.  To run
{\HLQE}, execute the following commands in {\HLQE} top-level directory
named \texttt{hol\_light}:
...
{}\unsure{Jacques and Patrick, are these instructions accurate?}

{}\unsure{Do we need to reference particular {\HLQE} files in this section?}
...
\texttt{epsilon} in {\HLQE}.  Since types are components of terms in
       {\HL}, an inductive type \texttt{type} of syntax values for
       {\HLQE} types (which are the same as {\HOL} types) is also
       needed.  The following are the definitions of \texttt{type} and
       \texttt{epsilon} in {\HLQE} as inductive types:
=======
syntactic values that represent the syntax trees of
eval-free expressions.  $\epsilon$ is defined as an inductive type
\texttt{epsilon}.  \texttt{type} and \texttt{epsilon} are defined
...
{\HLQE} terms that are eval-free (i.e., do not contain evaluations).
...
We said in subsection~\ref{subsec:cttqe-proof-system} that the proof system
for {\churchqe} is obtained by extending the Andrews' proof system for
{\qzero} with additional axioms named B1--B13.  Since {\qzero} and
...
an inductive type in {\HL}.  Third, Axiom B9 is implemented directly
...
\bsp
We present in this section two examples that test the implementation
...
The statement of LEM in {\churchqe} can directly written in {\HLQE} as
\begin{lstlisting}
`!x:epsilon. isExprType (x:epsilon) (TyBase "bool")
   ==> ((eval x to bool) \/ ~(eval x to bool))`
\end{lstlisting}
and readily proved in {\HLQE}.  An instance of this theorem is obtain
by applying the rule of inference \texttt{INST} followed by the
...
predicate $\mname{is-presburger}_{\epsilon \tarrow o}$.  Hence it is
possible to directly express both first-order Peano arithmetic and
Presburger arithmetic as theories in {\churchqe}.

\bsp
The induction schema for Peano arithmetic can be directly written in
{\HLQE} as:
\begin{lstlisting}
`!f:epsilon. 
   (isExprType 
...
This statement named \texttt{peanoInduction} is proved in {\HLQE} by:

\bsp
\be

  \item Instantiating the {\HL} theorem
    \[\texttt{`!P. P(\_0) /\ (!n. P(n) ==> P(SUC n)) ==> !n. P n`}\]
    that expresses the induction axiom with the predicate
  ...
  \item Reduce \texttt{peanoInduction} and then prove the result by
    instantiating \texttt{`P:num->bool`} in \texttt{indinst} with
    \texttt{`eval (f:epsilon) to (num->bool)`} using the
    \texttt{INST} which requires the theorem \texttt{nei\_peano}.

\ee
\esp

This example shows that the Induction Schema for Peano arithmetic ---
as well as the Induction Schema for Presburger arithmetic --- can be
expressed in {\HLQE}.  This enables us to define the first-order
theories of Peano arithmetic and Presburger arithmetic in {\HLQE}.
We do not know of any way to express these schemas or define these
first-order theories in {\HL} without adding new rules of inference to
it kernel.
...
{\HLQE} enables the user to directly reasoning about the syntax of the
terms of the logic.  We have shown that {\HLQE} can express a formula
schema, such as the induction schema, as a single formula that can be
instantiated and proved.  We also believe {\HLQE} can be used to
define, apply, and prove properties about SBMAs.  These benefits are
obtained at relatively low cost.  {\HLQE} works in exactly the same
way as {\HL} except that, in the presence of evaluations, the
instantiation of free variables may require proving side conditions
that say (1) a variable is not effective in a term or (2) that a
variable represented by a construction is not free in a term
represented by a construction (see
subsections~\ref{subsec:cttqe-proof-system}
...
be effectively used to reason about SBMAs.  In particular, we intend
to formalize in {\HLQE} the example on the symbolic differentiation of
polynomials in {\churchqe} that is presented in~\cite{FarmerArxiv16}.
This will require defining the algorithm for symbolic differentiation
of polynomials, writing a \emph{meaning formula} that specifies the
algorithms mathematical meaning, and finally proving the meaning
formula from the algorithm's definition and standard facts about
derivatives.
