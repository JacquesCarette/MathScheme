...
assistant.  Here we document our design and the challenges that needed
to be overcome.
\end{abstract}
...
object-level reasoning. Quotation gives a form of
\emph{reification} of object-level values which allows
introspection.  Along with inference rules, this gives a certain 
amount of \emph{logical reflection}. Evaluation adds to this
some aspects of \emph{computational reflection}.
...
\be
  ...
  \item The set of {\HL} terms was extended so that {\churchqe}
    ...
    contructors for quotations and evaluations.  See
  ...
  \item The {\HL} proof system was modified to include the machinery
    ...
    the {\HL} \texttt{INST} rule of inference that similtaneously
...
chosen {\HOL} type variable.  The other logical constants of
{\churchqe}~\cite[Table 1]{FarmerArxiv16} are mapped to {\HOL}
functions.
