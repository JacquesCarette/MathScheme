...
programming language.  The dual formalization, contrasting the two 
...
of transformers \emph{syntax-based mathematical algorithms
  (SBMAs)}~\cite{Farmer13}.  Examples includes algorithms that apply
...
implemented as a tactic that applies the axioms to $f(e_1,\ldots,e_)$
...
theories, BT1 and BT5 are theories of $0$ and $S$; BT2 and BT6 are
theories of $0$, $S$, and $+$; and BT3, BT4, and BT7 are theories of
$0$, $S$, $+$, and $*$, Several other biform theories could be added
to this graph, most notably Skolem arithmetic, the complete theory of
$0$, $S$, and $*$, which has a very complicated
axiomatization~\cite{Smorynski91}.  The details of each theory is
given below.
...
algorithm for adding binary numbers.
\end{biformthy}
...
\textbf{A}_\alpha$ where $\textbf{A}_\alpha$ is defined.
...
elided the following components of BT6 that are redundant or are
subsumed by other components: Constants
...
$\pi_{13}$.  Expressions of type~$\epsilon$, i.e., expressions that
denote constructions, are colored red to identify where reasoning
about syntax occurs.
...
be checked using $\pi_{15}$.

This biform theory has a defined constant $\mname{bnat}_{\iota \tarrow
  \iota \tarrow {\sf nat}}$ with the usual base~2 notation for
expressing natural numbers in a binary form.  There is a constant
$\mname{BPLUS}_{\epsilon \tarrow \epsilon \tarrow \epsilon}$ specified
by axioms 5--15 for adding quotations of these binary natural numbers.
$\mname{BPLUS}_{\epsilon \tarrow \epsilon \tarrow \epsilon}$ is
implemented by transformers $\pi_3$ and $\pi_4$.  $\pi_3$ is some
efficient algorithm implemented outside of {\churchuqe}, and $\pi_4$
is an algorithm that uses axioms 5--15 as conditional rewrite rules.
$\pi_4$ is sound if the rewriting mechanism is sound.  Showing the
soundness of $\pi_3$ would require a formal verification of its
program.  The meaning formula for $\mname{BPLUS}_{\epsilon \tarrow
  \epsilon \tarrow \epsilon}$, theorem 3, follows from axioms 5--15.
